...
const RejectedLeaves = () => {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(() => {
    const saved = localStorage.getItem('rejectedLeaves_itemsPerPage');
  ...
  useEffect(() => {
    ...
    // HARDCODED FILTER FOR REJECTED ONLY
  ...
  const downloadExcel = () => {
    const rows = filteredLeaves.map((l) => {
      ...
      return {
        ...
        Days: l.days || 0,
        Reason: l.reason || "",
        Status: l.status || "rejected",
      };
    });
    const ws = XLSX.utils.json_to_sheet(rows);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Rejected Leaves");
    XLSX.writeFile(wb, `Rejected_Leaves_${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
  ...
  const handleItemsPerPageChange = (e) => {
    ...
    localStorage.setItem('rejectedLeaves_itemsPerPage', String(newValue));
    setCurrentPage(1);
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          <div className="w-10 h-10 mx-auto mb-3 border-3 border-gray-200 border-t-red-600 rounded-full animate-spin"></div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden mb-6">
          {filteredLeaves.length === 0 ? (
            <div className="py-12 text-center text-sm text-gray-500 font-medium">
              No rejected leave records found matching current filter values.
            </div>
          ) : (
            <>
              {/* Desktop Table View */}
                ...
                <table className="min-w-full divide-y divide-gray-200 bg-white">
                  ...
                  <tbody className="divide-y divide-gray-200 text-xs">
                    {currentItems.map((l) => {
                      const empDetails = getEmployeeDetails(l.employeeId);
                      return (
                        <tr key={l._id} className="hover:bg-gray-50 transition-all">
