...
const LeavesList = () => {
  ...
  const StatCard = ({ icon: Icon, label, value, color }) => (
    <div className={`bg-white rounded-lg p-3 shadow-sm border-t-4 ${color} cursor-pointer hover:shadow-md transition-all duration-300 flex items-center justify-between`}>
      <div className="flex items-center gap-2"><Icon className="text-gray-400 text-base flex-shrink-0" /><div className="text-sm font-medium text-gray-700">{label}</div></div>
      <div className="text-sm font-bold text-gray-800"><CountUp end={value} duration={2} separator="," /></div>
    </div>
  );
    ...
    <div className="min-h-screen px-2 py-2 bg-gradient-to-br from-purple-50 to-blue-100">
      <div className="mx-auto max-w-9xl">
        ...
        <div className="grid grid-cols-2 gap-2 mb-2 sm:grid-cols-5">
          <StatCard icon={FiList} label="Total Requests" value={leaves.length} color="border-purple-500" />
          <StatCard icon={FiClock} label="Pending" value={leaves.filter(l => l.status === "pending").length} color="border-yellow-500" />
          <StatCard icon={FiCheckCircle} label="Approved" value={leaves.filter(l => l.status === "approved" && !l.compOffStatus?.exists).length} color="border-green-500" />
          <StatCard icon={FiXCircle} label="Rejected" value={leaves.filter(l => l.status === "rejected").length} color="border-red-500" />
          <StatCard icon={FaExchangeAlt} label="Comp-off" value={compOffRequests.length} color="border-purple-500" />
        </div>
