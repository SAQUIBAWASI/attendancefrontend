// CustomerReferralManagement.js - Customer Referrals Only
...
export default function CustomerReferralManagement() {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(() => {
    const saved = localStorage.getItem("customerReferral_itemsPerPage");
  ...
  const fetchReferrals = async () => {
    ...
    try {
      ...
      if (customerReferrals.length === 0) {
        ...
        setReferrals([]);
        showToast("No customer referrals found.", "info");
      } else {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this customer referral record?")) return;
    try {
      await deleteReferral(id);
      setReferrals((prev) => prev.filter((r) => r._id !== id));
      showToast("Customer referral deleted successfully", "info");
    } catch (err) {
  ...
  const downloadCSV = () => {
    if (filteredReferrals.length === 0) {
      alert("No customer referral records available to download!");
      return;
    }
    ...
    a.href = url;
    a.download = `customer_referral_records_${new Date().toLocaleDateString().replace(/\//g, "-")}.csv`;
    a.click();
    URL.revokeObjectURL(url);
    showToast(`Exported ${filteredReferrals.length} customer referral records to CSV!`);
  };
  ...
  const handleItemsPerPageChange = (e) => {
    ...
    localStorage.setItem("customerReferral_itemsPerPage", String(newValue));
    setCurrentPage(1);
  };
    ...
    return (
      <div className="emp-dash">
        <div className="emp-dash__loading">
          ...
          <p className="emp-dash__loading-text">Loading customer referrals...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="lg:hidden flex flex-col gap-2 mb-3">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <h1 className="text-base font-bold whitespace-nowrap">
              Customer <span className="text-indigo-600">Referrals</span>
            </h1>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4 mb-6">
          ...
          >
            <div className="emp-dash__stat-top">
              <span className="emp-dash__stat-label">Total Customers</span>
              <div className="emp-dash__stat-icon emp-dash__stat-icon--rate">
                <FiUsers />
              </div>
            </div>
            <div className="emp-dash__stat-value">{stats.total}</div>
            <div className="emp-dash__stat-meta">all customer referrals</div>
          </div>
            ...
            <div className="bg-white rounded-2xl max-w-lg w-full p-6 md:p-8 shadow-2xl border border-gray-200 relative max-h-[90vh] overflow-y-auto">
              ...
              <div className="my-5 bg-gray-50 p-5 rounded-xl border border-gray-200 space-y-4">
                ...
                <div className="flex items-center gap-3.5 pb-3 border-b border-gray-200">
                  <div className="w-12 h-12 rounded-full bg-indigo-500 text-white font-bold text-lg flex items-center justify-center flex-shrink-0 shadow-inner">
                    {selectedReferral.customerName?.charAt(0).toUpperCase() || "C"}
                  </div>
                ...
                <div>
                  ...
                  <div className="text-xs font-medium text-gray-700 mt-0.5 flex items-start gap-1">
                    ...
                    <span>{selectedReferral.customerAddress || "N/A"}</span>
                  </div>
                </div>
