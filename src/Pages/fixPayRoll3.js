...
const p1Start = `if (salaryData.success && salaryData.salaries.length > 0) {
        processedSalaries = salaryData.salaries.map(emp => {`;
const p1EndSnippet = `calculatedSalary = Math.max(0, calculatedSalary - weekOffAmount);
          }`;

const start1 = content.indexOf(p1Start);
let end1 = content.indexOf(p1EndSnippet, start1);
...
if (start1 !== -1 && end1 !== -1) {
  ...
  content = content.substring(0, start1) + newProcessed1 + content.substring(end1 + p1EndSnippet.length);
} else {
  console.log("Failed processed 1 replacement");
}
