...
//   const fetchRejectedVisits = useCallback(async () => {
//     setLoading(true);
//     setError('');
//     try {
//       const res = await axios.get(`${API_BASE_URL}/call-data/rejected`);
//       if (res.data.success) setVisits(res.data.data || []);
//     } catch (err) {
//       console.error('Fetch rejected visits error:', err);
//       setError('Failed to load rejected visits. Please try again.');
//     } finally { setLoading(false); }
//   }, []);

//   useEffect(() => { fetchRejectedVisits(); }, [fetchRejectedVisits]);
...
//     link.setAttribute('download', `rejected-visits-${selectedMonth || 'all'}.csv`);
...
//           <div>
//             <h1 className="emp-dash__greeting">Rejected <span>Visits</span></h1>
//             <p className="emp-dash__subtitle">
//               View all rejected visit records across the organization.
//             </p>
//           </div>
...
//               </div>
//               {/* Refresh */}
//               <button onClick={() => { setSearchQuery(''); setSelectedMonth(''); fetchRejectedVisits(); }}
...
//               <strong className="text-gray-800">{visitsForMonth.length}</strong> rejected records for{' '}
...
//           <div className="emp-dash__card-header">
//             <div>
//               <h3 className="emp-dash__card-title">Rejected Visit Records</h3>
//               <p className="emp-dash__card-desc">All rejected visits for <strong>{getPeriodLabel()}</strong>.</p>
//             </div>
//           </div>
...
//                       <span className="text-sm font-medium text-gray-500">Loading rejected visits…</span>
//                     </div>
//                   </td></tr>
//                 ) : filtered.length === 0 ? (
...
//                       <p className="text-gray-500 font-semibold text-sm">No rejected visits found</p>
...
//                 Showing <span className="text-gray-900 font-bold">{filtered.length}</span> rejected records for{' '}
//                 <span className="text-blue-600 font-bold">{getPeriodLabel()}</span>
//               </p>
//             </div>
//           )}
...
import {
  FaPhone, FaLink, FaSearch, FaTimes, FaSync,
  FaMapMarkerAlt, FaUser, FaCalendarAlt, FaCommentAlt,
  FaDownload, FaTimesCircle, FaChevronUp, FaChevronDown
} from 'react-icons/fa';
import { FiFilter, FiCalendar, FiActivity, FiTrendingUp, FiTrash2, FiRefreshCw } from 'react-icons/fi';
import './EmployeeDashboard.css';
...
const RejectedVisits = () => {
  ...
  const fetchRejectedVisits = useCallback(async () => {
    ...
    setError('');
    try {
      const [rejectedRes, allRes] = await Promise.all([
        axios.get(`${API_BASE_URL}/call-data/rejected`),
        axios.get(`${API_BASE_URL}/call-data/all`)
      ]);
      if (rejectedRes.data.success) setVisits(rejectedRes.data.data || []);
      ...
      <main className="p-4 sm:p-6 lg:p-8">
        ...
        <div className="hidden lg:flex items-center justify-between gap-3 flex-wrap mb-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Refresh Button */}
            <button
              onClick={() => { setSearchQuery(''); fetchRejectedVisits(); }}
              disabled={loading}
        ...
        <div className="lg:hidden flex items-center justify-between gap-2 flex-wrap mb-3">
          <div>
            <h1 className="text-base font-bold whitespace-nowrap">
              Rejected <span className="text-indigo-600">Visits</span>
            </h1>
          </div>
        ...
        <div className="lg:hidden mb-3">
          <div className="flex items-center justify-between p-2.5 bg-white rounded-xl border border-gray-200">
            ...
            <span className="text-xs text-gray-500">
              <strong>{filtered.length}</strong> rejected visits
            </span>
          </div>
          ...
          {!loading && filtered.length > 0 && (
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-4 sm:px-6 py-3 border-t border-gray-100 bg-gray-50/50">
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
                <span>Rejected: <strong className="text-red-500">{filtered.length}</strong></span>
              </div>
            </div>
          )}
        </div>
