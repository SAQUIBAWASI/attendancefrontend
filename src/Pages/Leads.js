...
//     setError('');
//     try {
//       const res = await axios.get(`${API_BASE_URL}/call-data/leads`);
...
//           <div>
//             <h1 className="emp-dash__greeting"><span>Leads</span></h1>
//             <p className="emp-dash__subtitle">
//               View all lead records across the organization.
//             </p>
//           </div>
...
//               </div>
//               {/* Refresh */}
//               <button onClick={() => { setSearchQuery(''); setSelectedMonth(''); fetchLeads(); }}
...
//               <strong className="text-gray-800">{visitsForMonth.length}</strong> lead records for{' '}
...
//           <div className="emp-dash__card-header">
//             <div>
//               <h3 className="emp-dash__card-title">Lead Records</h3>
//               <p className="emp-dash__card-desc">All leads for <strong>{getPeriodLabel()}</strong>.</p>
//             </div>
//           </div>
...
//                       <span className="text-sm font-medium text-gray-500">Loading leads…</span>
//                     </div>
//                   </td></tr>
//                 ) : filtered.length === 0 ? (
...
//                 Showing <span className="text-gray-900 font-bold">{filtered.length}</span> lead records for{' '}
//                 <span className="text-blue-600 font-bold">{getPeriodLabel()}</span>
//               </p>
//             </div>
//           )}
...
//     setError('');
//     try {
//       const [leadsRes, allRes] = await Promise.all([
//         axios.get(`${API_BASE_URL}/call-data/leads`),
//         axios.get(`${API_BASE_URL}/call-data/all`)
//       ]);
//       if (leadsRes.data.success) setVisits(leadsRes.data.data || []);
...
//     link.setAttribute('download', `leads-${selectedMonth || 'all'}.csv`);
...
//                 <span>Leads: <strong className="text-green-600">{filtered.length}</strong></span>
//               </div>
//             </div>
//           )}
//         </div>
      ...
      <main className="p-4 sm:p-6 lg:p-8">
        ...
        <div className="hidden lg:flex items-center justify-between gap-3 flex-wrap mb-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Refresh Button */}
            <button
              onClick={() => { setSearchQuery(''); fetchLeads(); }}
              disabled={loading}
