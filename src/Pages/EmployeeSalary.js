...
//         {/* Header */}
//         {/* <div className="mb-3">
//           <h1 className="text-lg font-bold text-gray-700 sm:text-xl md:text-2xl">💰 My Salary Records</h1>
//           <p className="text-xs text-gray-500 sm:text-sm">View and download your salary slips</p>
//         </div> */}
...
//               )}
//               {searchTerm && (
//                 <div className="flex items-center gap-1 px-2 py-0.5 text-xs text-gray-700 bg-gray-100 rounded-full">
...
//                 <span className="text-gray-500">Approved Leaves</span>
...
//   // Calculate Daily Rate
//   const calculateDailyRate = (employee, monthlySalary) => {
//     const salary = monthlySalary || employee?.salaryPerMonth || 0;
...
//       const employeeData = getCurrentEmployee();
//       const employeeId = employeeData?.employeeId;
//       const employeeDbId = employeeData?._id;
...
//       const isHistorical = isHistoricalMonth(monthToFetch);
//       const isCurrent = isCurrentMonth(monthToFetch);
//       const includeWeekOff = shouldIncludeWeekOffInSalary(monthToFetch);
//       const canDownload = isPayslipDownloadAllowed(monthToFetch);

//       setMonthInfo({ isHistorical, isCurrent, includeWeekOff, canDownload });

//       // Fetch salary from employee API (with increment logic)
//       const targetDate = new Date(monthToFetch.split('-')[0], monthToFetch.split('-')[1] - 1, 15);
...
//       if (employeeDbId) {
//         try {
//           const salaryRes = await fetch(`${BASE_URL}/api/employees/${employeeDbId}/salary-for-date?date=${formattedDate}`);
//           if (salaryRes.ok) {
//             const salaryData = await salaryRes.json();
//             if (salaryData.success && salaryData.data) {
//               salaryForMonth = salaryData.data.salaryPerMonth;
//               effectiveFrom = salaryData.data.effectiveFrom || null;
//               originalSalary = salaryData.data.originalSalary || 0;
//               hasIncrement = salaryData.data.hasIncrement || false;
//             }
//           }
//         } catch (err) {
//           console.warn(`Failed to fetch salary:`, err.message);
//         }
//       }

//       // Fallback to master data
//       if (salaryForMonth === 0 && employeesMasterData[employeeId]) {
//         salaryForMonth = employeesMasterData[employeeId].salaryPerMonth || 0;
//         originalSalary = employeesMasterData[employeeId].originalSalary || salaryForMonth;
//       }

//       // Fetch attendance summary
//       const salaryUrl = `${BASE_URL}/api/attendancesummary/getsalaries?month=${monthToFetch}`;
//       const salaryRes = await fetch(salaryUrl, { signal: abortControllerRef.current.signal });
...
//       // Fetch leaves data
//       const leavesRes = await fetch(`${BASE_URL}/api/leaves/leaves?status=approved`, { signal: abortControllerRef.current.signal });
//       let leavesData = leavesRes.ok ? await leavesRes.json() : [];
      
//       const processedLeaves = processLeavesData(leavesData, monthToFetch);
//       const compOffMap = await processCompOffData(monthToFetch, processedLeaves);

//       let employeeSalaryRecords = [];

//       if (salaryData.success && salaryData.salaries && salaryData.salaries.length > 0) {
//         const employeeSalary = salaryData.salaries.find(salary => salary.employeeId === employeeId);
        
//         if (employeeSalary) {
//           const actualWeekOffDays = employeeSalary.weekOffs || 0;
//           let calculatedSalary = employeeSalary.calculatedSalary || 0;
          
//           if (!includeWeekOff && calculatedSalary > 0 && salaryForMonth > 0) {
//             const daysInMonth = salaryData.monthDays || 30;
//             const dailyRate = salaryForMonth / daysInMonth;
//             const weekOffAmount = actualWeekOffDays * dailyRate;
//             calculatedSalary = Math.max(0, calculatedSalary - weekOffAmount);
//           }

//           const compOffData = compOffMap[employeeId] || { earned: 0, used: 0, balance: 0 };
//           let finalCalculatedSalary = calculatedSalary;
          
//           if (compOffData.balance > 0 && salaryForMonth > 0) {
//             const daysInMonth = salaryData.monthDays || 30;
//             const dailyRate = salaryForMonth / daysInMonth;
//             finalCalculatedSalary += compOffData.balance * dailyRate;
//           }
...
//             isHistoricalMonth: isHistorical,
//             isCurrentMonth: isCurrent,
//             monthFormatted: formatMonthDisplay(monthToFetch),
//             halfDays: employeeSalary.halfDayWorking || 0,
//             workingDays: employeeSalary.totalWorkingDays || employeeSalary.workingDays || 0,
//             presentDays: employeeSalary.presentDays || 0,
//             totalLeaves: employeeSalary.totalLeaves || 0,
//             month: monthToFetch,
...
//           hasIncrement: hasIncrement,
//           effectiveFrom: effectiveFrom,
//           calculatedSalary: salaryForMonth,
...
//       setIsLoadingMonth(false);
//     }
//   }, [processLeavesData, processCompOffData, employeesMasterData, monthDays]);
...
//     const currentMonth = getCurrentMonth();
//     setSelectedMonth(currentMonth);
//     await fetchSalaryData(currentMonth);
//   };
...
//   // Get leave types for display
//   const getLeaveTypes = (employee) => {
//     const leavesData = employeeLeaves[employee.employeeId] || { CL: 0, EL: 0, COFF: 0, LOP: 0, Other: 0 };
//     const leaveStrings = [];
//     if (leavesData.CL > 0) leaveStrings.push(`CL: ${leavesData.CL}`);
//     if (leavesData.EL > 0) leaveStrings.push(`EL: ${leavesData.EL}`);
//     if (leavesData.COFF > 0) leaveStrings.push(`COFF: ${leavesData.COFF}`);
//     if (leavesData.LOP > 0) leaveStrings.push(`LOP: ${leavesData.LOP}`);
//     if (leavesData.Other > 0) leaveStrings.push(`Other: ${leavesData.Other}`);
//     return leaveStrings.length > 0 ? leaveStrings.join(', ') : 'No Leaves';
//   };
...
//     const employeeData = getEmployeeData(employee);
//     const totalMonthDays = employee.monthDays || monthDays || 30;
//     const dailyRate = parseFloat(calculateDailyRate(employee, employee.salaryPerMonth)) || 0;
...
//     let totalPaidDays = presentDays + (halfDays * 0.5) + actualWeekOffDays + compOffBalance;
//     const halfDayDeductionDays = halfDays * 0.5;
//     const halfDayDeductionAmount = halfDayDeductionDays * dailyRate;
//     const totalUnpaidDays = Math.max(0, totalMonthDays - totalPaidDays);
//     const lopDays = Math.max(0, totalUnpaidDays - halfDayDeductionDays);
    
//     const grossSalary = employee.salaryPerMonth || 0;
//     const bonus = employee.extraWork?.bonus || 0;
//     const extraDaysPay = (employee.extraWork?.extraDays || 0) * dailyRate;
//     const totalEarnings = grossSalary + bonus + extraDaysPay + compOffPay;
    
//     const otherDeductions = employee.extraWork?.deductions || 0;
//     const netPay = employee.calculatedSalary || (totalEarnings - (halfDayDeductionAmount + (lopDays * dailyRate) + otherDeductions));
//     const lopAmount = Math.max(0, totalEarnings - netPay - halfDayDeductionAmount - otherDeductions);
//     const totalDeductions = halfDayDeductionAmount + lopAmount + otherDeductions;
...
//             .section-header { text-align: center; padding: 5px; }
//           </style>
//         </head>
//         <body>
//           <div class="invoice-container">
...
//                 <td colspan="6" class="header-cell">
//                   <div style="display: flex; align-items: center; justify-content: space-between;">
//                     <div style="width: 200px;"><img src="${logo}" alt="Logo" style="height: 40px; width: auto; object-fit: contain; display: block;"></div>
//                     <div style="flex: 1; text-align: center;"><h2 style="margin: 0; font-size: 16px;">Timely Health Tech Pvt Ltd</h2></div>
//                     <div style="width: 200px;"></div>
//                   </div>
//                 </td>
//               </tr>
//               <tr><td colspan="6" class="section-header">Payment Slip For The Month Of - ${formatMonthDisplay(employee.month)}</td></tr>
//               <tr><td width="15%">Employee ID</td><td width="20%">${employee.employeeId}</td><td width="15%">Employee</td><td width="25%">${employee.name}</td><td width="10%">PAN No.</td><td width="15%">${employeeData.panNo || ''}</td></tr>
//               <tr><td>Joining Date</td><td>${employeeData.joiningDate ? new Date(employeeData.joiningDate).toLocaleDateString('en-GB') : '-'}</td><td>Department</td><td>${employeeData.department || employee.department || '-'}</td><td>Designation</td><td>${employeeData.designation || employee.designation || '-'}</td></tr>
//               <tr><td>Bank Account</td><td colspan="5">${employeeData.bankAccount || ''}</td></tr>
//             </table>
//             <table style="margin-top: 10px;">
//               <tr style="background-color: #f0f0f0;"><th>Particulars</th><th>Days</th><th>Rate (₹/day)</th><th>Amount (₹)</th></tr>
//               <tr><td>Monthly Salary</td><td>${totalMonthDays}</td><td>${dailyRate.toFixed(2)}</td><td>${grossSalary.toFixed(2)}</td></tr>
//               ${actualWeekOffDays > 0 ? `<tr><td>Week Off Days</td><td>${actualWeekOffDays}</td><td>${dailyRate.toFixed(2)}</td><td>${(actualWeekOffDays * dailyRate).toFixed(2)}</td></tr>` : ''}
//               ${compOffBalance > 0 ? `<tr><td>Comp-off Balance</td><td>${compOffBalance}</td><td>${dailyRate.toFixed(2)}</td><td>${compOffPay.toFixed(2)}</td></tr>` : ''}
//               ${bonus > 0 ? `<tr><td>Bonus</td><td>-</td><td>-</td><td>${bonus.toFixed(2)}</td></tr>` : ''}
//               ${lopAmount > 0 ? `<tr style="color: red;"><td>LOP/Absent Deduction</td><td>${lopDays}</td><td>${dailyRate.toFixed(2)}</td><td>-${lopAmount.toFixed(2)}</td></tr>` : ''}
//               ${halfDayDeductionAmount > 0 ? `<tr style="color: red;"><td>Half Day Deduction</td><td>${halfDays}</td><td>${(dailyRate / 2).toFixed(2)}</td><td>-${halfDayDeductionAmount.toFixed(2)}</td></tr>` : ''}
//               ${otherDeductions > 0 ? `<tr style="color: red;"><td>Other Deductions</td><td>-</td><td>-</td><td>-${otherDeductions.toFixed(2)}</td></tr>` : ''}
//               <tr style="font-weight: bold; background-color: #e0e0e0;"><td colspan="3">Net Payable</td><td>₹${netPay.toFixed(2)}</td></tr>
//               <tr><td colspan="4"><strong>Net Payable (In words):</strong> ${numberToWords(netPay)}</td></tr>
//               <tr><td colspan="4" style="font-size: 9px; color: #666;">Note: This is a System generated slip and does not require company sign and stamp.</td></tr>
//             </table>
//           </div>
//         </body>
//       </html>
...
//   const downloadSalarySlip = async (employee) => {
//     if (!employee.canDownload) {
//       const daysInMonth = getDaysInMonth(employee.month);
//       alert(`Salary slip for current month will be available for download from ${daysInMonth}th onwards.`);
//       return;
//     }
//     const slipContent = generateInvoiceHTML(employee);
//     const printWindow = window.open('', '_blank');
...
//         {/* Month Info Notice */}
//         {monthInfo.isHistorical ? (
//           <div className="px-3 py-2 mb-4 rounded-md shadow-sm bg-green-50 border-l-4 border-green-500">
//             <p className="text-xs font-medium text-green-700">✓ Historical Month - Full salary with week-off included</p>
//           </div>
//         ) : monthInfo.isCurrent ? (
//           monthInfo.includeWeekOff ? (
//             <div className="px-3 py-2 mb-4 rounded-md shadow-sm bg-green-50 border-l-4 border-green-500">
//               <p className="text-xs font-medium text-green-700">✓ Current Month (After 26th) - Week-off included in salary</p>
//             </div>
//           ) : (
//             <div className="px-3 py-2 mb-4 rounded-md shadow-sm bg-yellow-50 border-l-4 border-yellow-500">
//               <p className="text-xs font-medium text-yellow-700">⚠️ Current Month (Before 26th) - Week-off will be added after 26th</p>
//             </div>
//           )
//         ) : null}
...
//                       <th className="py-2.5 px-2">LEAVES</th>
//                       <th className="py-2.5 px-2">COMP-OFF</th>
//                       <th className="py-2.5 px-2">MONTHLY SALARY</th>
//                       <th className="py-2.5 px-2">CALCULATED</th>
//                       <th className="py-2.5 px-2">STATUS</th>
//                       <th className="py-2.5 px-2">ACTIONS</th>
//                     </tr>
//                   </thead>
//                   <tbody className="divide-y divide-gray-200">
//                     {currentRecords.map((emp, idx) => {
//                       const dailyRate = calculateDailyRate(emp, emp.salaryPerMonth);
...
//                               <div className="flex flex-col items-center">
//                                 <span className="px-1.5 py-0.5 text-xs font-medium text-purple-700 bg-purple-100 rounded-full">
//                                   +{emp.compOffEarned} / -{emp.compOffUsed} = {compOffBalance}
//                                 </span>
//                                 <div className="text-[8px] text-gray-500">₹{((compOffBalance) * parseFloat(dailyRate)).toFixed(0)}</div>
//                               </div>
//                             ) : emp.compOffEarned > 0 ? (
//                               <span className="px-1.5 py-0.5 text-xs font-medium text-purple-700 bg-purple-100 rounded-full">
//                                 +{emp.compOffEarned} / -{emp.compOffUsed} = 0
...
//                           <td className="px-2 py-1.5 text-center">
//                             <div className="flex flex-col items-center">
//                               <span className="text-xs font-semibold text-blue-600">₹{(emp.salaryPerMonth || 0).toLocaleString()}</span>
//                               {showOldSalary && (
//                                 <div className="flex flex-col items-center">
...
//                             </td>
//                           <td className="px-2 py-1.5 text-center">
//                             <div className="text-xs font-semibold text-green-700">₹{Math.round(emp.calculatedSalary || 0).toLocaleString()}</div>
//                             <div className="text-[8px] text-gray-500">₹{dailyRate}/day</div>
//                             </td>
//                           <td className="px-2 py-1.5 text-center">
...
//                               <button onClick={() => downloadSalarySlip(emp)} disabled={!emp.canDownload} className={`p-1 rounded-md ${emp.canDownload ? 'text-purple-600 hover:bg-purple-50' : 'text-gray-500 cursor-not-allowed'}`}><Download size={14} /></button>
//                             </div>
//                             </td>
//                         </tr>
//                       );
...
//                   <p><span className="font-medium text-gray-700">Department:</span> {selectedEmployee.department || 'N/A'}</p>
//                   <p><span className="font-medium text-gray-700">Designation:</span> {selectedEmployee.designation || 'N/A'}</p>
//                   <p><span className="font-medium text-gray-700">Month:</span> {selectedEmployee.month || selectedMonth || getCurrentMonth()} ({selectedEmployee.monthDays || monthDays} days)</p>
//                 </div>
//               </div>
//             </div>
...
//                   <p className="text-xl font-bold text-blue-600">₹{(selectedEmployee.salaryPerMonth || 0).toLocaleString()}</p>
//                   {selectedEmployee.hasIncrement && selectedEmployee.originalSalary && selectedEmployee.originalSalary !== selectedEmployee.salaryPerMonth && (
//                     <p className="text-xs text-gray-500 line-through">₹{(selectedEmployee.originalSalary || 0).toLocaleString()}</p>
//                   )}
//                 </div>
//                 <div>
...
//               <div className="flex justify-between pb-1 border-b"><span className="text-gray-500">Daily Rate</span><span className="font-semibold text-gray-700">₹{calculateDailyRate(selectedEmployee, selectedEmployee.salaryPerMonth)}/day</span></div>
//               <div className="flex flex-col pb-2 border-b sm:col-span-2">
...
//                 <div className="flex justify-between"><span className="font-medium text-gray-500">Comp-off Balance</span><span className="font-semibold text-purple-600">{selectedEmployee.compOffBalance || 0} days</span></div>
//               </div>
//             </div>
//             <div className="flex justify-end space-x-3">
...
//             <p className="text-center text-sm font-bold text-gray-700">{value}</p>
//           </div>
//         </div>
//       </div>
//     );
...
//                   <p className="text-xs text-gray-500">Month: {selectedEmployee.monthFormatted || formatMonthDisplay(selectedEmployee.month)}</p>
//                   <p className="text-xs text-gray-500">Days in Month: {selectedEmployee.monthDays || 30}</p>
//                 </div>
//               </div>
//             </div>
...
//               </div>
//               <div className="p-2 bg-white border rounded-lg">
//                 <p className="text-xs text-gray-500">WeekOff Days</p>
