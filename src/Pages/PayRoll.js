...
//   const SALARY_API_URL = "http://localhost:5000/api/employees/getsalaries";
...
//     // STEP 0 → Fetch Attendance Summary Data First
//     let summaryData = [];
//     try {
//       const summaryRes = await fetch(ATTENDANCE_SUMMARY_API_URL);
//       if (summaryRes.ok) {
...
//     // STEP 1 → Salary API (Main Salary Info)
//     let salaryData = { success: false, salaries: [] };
//     try {
//       const salaryRes = await fetch(SALARY_API_URL);
//       if (salaryRes.ok) {
...
//           workingDays: summary.totalWorkingDays ?? emp.workingDays ?? 0,
//           totalWorkingDays: summary.totalWorkingDays ?? 0,
//           halfDayWorking: summary.halfDayWorking ?? 0,
...
//           month: summary.month ?? emp.month,

//           salaryPerDay: emp.salaryPerDay || (emp.salaryPerMonth ? emp.salaryPerMonth / 30 : 0)
//         };
//       });
//     } else {
...
//         calculatedSalary: 0,
//         salaryPerMonth: emp.salaryPerMonth || 0,
//         salaryPerDay: emp.salaryPerMonth ? emp.salaryPerMonth / 30 : 0,
...
//     setEmployeeLeaves(leavesMap);
//     console.log("🍃 Processed leaves data:", leavesMap);
//   };
...
//   const calculateSalary = (employee) => {
//     // Use backend calculated salary first
//     if (employee.calculatedSalary && employee.calculatedSalary > 0) {
//       return employee.calculatedSalary;
//     }
    
//     // Fallback calculation (same logic as backend)
//     const employeeData = employeesMasterData[employee.employeeId];
//     if (!employeeData?.salaryPerMonth) return 0;
    
//     const dailyRate = employeeData.salaryPerMonth / 30;
//     const presentDays = employee.presentDays || 0;
//     const halfDays = employee.halfDayWorking || 0;
//     const weekOffs = employee.weekOffs || employeeData.weekOffPerMonth || 0;
//     const totalLeaves = employee.totalLeaves || 0;
    
//     // Same logic as backend: (present + half_days*0.5 + weekoff) - leaves
//     const paidDays = Math.max(0, presentDays + (halfDays * 0.5) + weekOffs - totalLeaves);
//     return Math.round(paidDays * dailyRate);
//   };
...
//   const calculateHolidays = (employee) => {
//     return 2; // Fixed holidays per month
//   };
...
//   const fetchAttendanceDetails = async (employeeId) => {
//     try {
//       const response = await fetch(ATTENDANCE_DETAILS_API_URL);
//       const data = await response.json();
...
//     const paidDays = (editFormData.workingDays || 0) + weekOffDays + holidays + (0.5 * (editFormData.halfDayWorking || 0)) - unpaidLeaves;
//     const effectivePaidDays = Math.max(0, paidDays);
//     const baseSalary = effectivePaidDays * dailyRate;
...
//   // ✅ Generate invoice HTML with WeekOff Calculation
//   const generateInvoiceHTML = (employee) => {
//     const employeeData = getEmployeeData(employee);
...
//     const weekOffAmount = weekOffDays * dailyRate;
//     const holidaysAmount = holidays * dailyRate;
//     const halfDaysAmount = (employee.halfDayWorking || 0) * (dailyRate / 2);
//     const unpaidLeavesDeduction = unpaidLeaves * dailyRate;
...
//                   <td>₹${Math.round(holidaysAmount)}</td>
//                 </tr>
//                 ${(employee.halfDayWorking || 0) > 0 ? `
//                 <tr>
//                   <td>Half Days (0.5 day each)</td>
//                   <td>${employee.halfDayWorking}</td>
//                   <td>₹${Math.round(dailyRate / 2)}/half-day</td>
//                   <td>₹${Math.round(halfDaysAmount)}</td>
//                 </tr>
//                 ` : ''}
...
//                 <tr>
//                   <td>Effective Paid Days</td>
//                   <td>${(employee.workingDays || 0) + weekOffDays + holidays + (0.5 * (employee.halfDayWorking || 0)) - unpaidLeaves}</td>
//                 </tr>
//                 <tr class="total-row">
//                   <td><strong>Final Salary</strong></td>
//                   <td><strong>₹${Math.round(totalSalary)}</strong></td>
...
//   const getLeaveTypes = (employee) => {
//     // First try to get from employee data (from backend)
...
const PayRoll = () => {
  ...
  const handleEditSubmit = (e) => {
    ...
    const paidDays = Math.max(0, effectiveWorkingDays + weekOffDays + holidays - unpaidLeaves);
  ...
  const generateInvoiceHTML = (employee) => {
    ...
    // Total salary = (Working days + WeekOffs + Holidays - Unpaid leaves) * Daily rate + Extra work
    const totalSalary = workingDaysAmount + weekOffAmount + holidaysAmount - unpaidLeavesDeduction + extraWorkAmount;
          ...
          <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
            ...
            <div className="flex gap-3">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (monthValue) {
                      ...
                      // Clear filter, load current month
                      fetchData();
                    }
                  }}
              ...
              <button
                ...
                className="px-4 py-3 font-semibold text-gray-700 transition duration-200 bg-gray-100 border border-gray-300 rounded-lg hover:bg-gray-200"
              >
                Current Month
              </button>
              ...
              <button
                ...
                className="flex items-center px-6 py-3 font-semibold text-white transition duration-200 bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoadingMonth ? (
                  <>
...
//     // Debug: Check months in data
//     console.log("✅ FINAL PROCESSED SALARIES:", processedSalaries);
//     console.log("✅ Months in data:", [...new Set(processedSalaries.map(r => r.month))]);
...
// // ✅ Filter records based on search - EXACT MATCH ONLY
// useEffect(() => {
//   console.log("🔍 Searching for:", searchTerm);
//   console.log("📊 Total records:", records.length);
  
//   if (records.length > 0) {
//     console.log("📅 All months in data:", [...new Set(records.map(r => r.month))]);
//   }
...
//     if (record.employeeId?.toString().toLowerCase().includes(searchLower)) {
//       console.log(`✓ Matched ID: ${record.employeeId} (${record.month})`);
//       return true;
//     }
    
//     if (record.name?.toLowerCase().includes(searchLower)) {
//       console.log(`✓ Matched name: ${record.name} (${record.month})`);
//       return true;
//     }
    
//     const recordMonth = record.month?.toLowerCase();
//     if (!recordMonth) return false;
    
//     console.log(`Checking month match: "${recordMonth}" vs search: "${searchLower}"`);
...
//     let searchMonthNum = null;
    
//     for (const [monthName, monthNum] of Object.entries(monthMap)) {
//       if (monthName.includes(searchLower) || 
//           monthName.substring(0, 3) === searchLower) {
//         searchMonthNum = monthNum;
//         break;
//       }
//     }
    
//     if (!searchMonthNum && /^\d{1,2}$/.test(searchLower)) {
//       const num = parseInt(searchLower);
//       if (num >= 1 && num <= 12) {
//         searchMonthNum = num.toString().padStart(2, '0');
//       }
//     }
    
//     if (searchMonthNum) {
//       if (recordMonth.includes(`-${searchMonthNum}`)) {
//         console.log(`✓ EXACT MONTH MATCH: ${recordMonth} contains ${searchMonthNum}`);
//         return true;
//       }
      
//       if (recordMonth === searchMonthNum) {
//         console.log(`✓ DIRECT MONTH MATCH: ${recordMonth} === ${searchMonthNum}`);
//         return true;
//       }
//     }
    
//     if (recordMonth.includes(searchLower)) {
//       console.log(`✓ STRING MATCH: ${recordMonth} includes ${searchLower}`);
//       return true;
//     }
    
//     console.log(`✗ NO MATCH: ${recordMonth} doesn't match ${searchLower}`);
//     return false;
//   });
  
//   console.log(`✅ Filter result: ${filtered.length} records out of ${records.length}`);
//   if (filtered.length === 0 && searchTerm) {
//     console.log("⚠️ No records found! Check month format in data");
//   }
...
//                 <input
//                   type="text"
//                   placeholder="Search by ID, Name, or Month (e.g., EMP0015, John, November, 11)..."
//                   value={searchTerm}
//                   onChange={(e) => setSearchTerm(e.target.value)}
//                   className="w-full py-3 pl-10 pr-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
//                 />
...
//             {[
//               { name: "January", code: "01", num: "1" },
//               { name: "February", code: "02", num: "2" },
//               { name: "March", code: "03", num: "3" },
//               { name: "April", code: "04", num: "4" },
//               { name: "May", code: "05", num: "5" },
//               { name: "June", code: "06", num: "6" },
//               { name: "July", code: "07", num: "7" },
//               { name: "August", code: "08", num: "8" },
//               { name: "September", code: "09", num: "9" },
//               { name: "October", code: "10", num: "10" },
//               { name: "November", code: "11", num: "11" },
//               { name: "December", code: "12", num: "12" }
//             ].map((month) => (
//               <button
//                 key={month.name}
//                 onClick={() => {
//                   setSearchTerm(month.code);
//                 }}
//                 className={`px-4 py-2 text-sm font-medium rounded-lg border ${
//                   searchTerm === month.code || searchTerm === month.num
//                     ? 'text-white bg-blue-600 border-blue-600' 
//                     : 'text-gray-700 bg-gray-100 border-gray-300 hover:bg-gray-200'
//                 }`}
//                 title={`Show ${month.name} data`}
//               >
...
//                 <span className="font-medium">Search:</span> "{searchTerm || 'All'}" | 
//                 <span className="font-medium ml-2">Showing:</span> {filteredRecords.length} of {records.length} records
//               </p>
//               {records.length > 0 && (
//                 <p className="text-xs text-gray-500 mt-1">
//                   Available months: {[...new Set(records.map(r => r.month || 'No Month'))].join(', ')}
//                 </p>
//               )}
//             </div>
//             {searchTerm && (
