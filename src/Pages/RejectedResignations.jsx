...
const RejectedResignations = () => {
  ...
  const fetchResignations = async () => {
    try {
      ...
      const rejected = allApps.filter(
        (app) => app.status === "Resigned" && app.resignationStatus === "Rejected"
      );
      setResignations(rejected);
    } catch (err) {
  ...
  const downloadResignationPDF = (app) => {
    ...
    doc.save(`Resignation_Rejected_${app.firstName}_${app.lastName}.pdf`);
  };
  ...
  const exportToCSV = () => {
    ...
    a.href = url;
    a.download = `rejected_resignations_${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
    ...
    <div className="p-3 mx-auto bg-[#0a0a0a] rounded-lg shadow-md max-w-9xl min-h-screen">
      ...
      <div className="flex flex-col gap-4 mb-4 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-300">Rejected Resignations</h2>
            <p className="text-xs text-gray-500">{filteredData.length} rejected resignation(s)</p>
          </div>
          <span className="px-3 py-1 text-[10px] font-black rounded-full uppercase tracking-widest bg-red-100 text-red-700">
            Rejected
          </span>
        </div>
      ...
      <div className="overflow-x-auto bg-[#0a0a0a] shadow-lg rounded-xl">
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-500">
            ...
            <p className="font-medium">No rejected resignations found</p>
          </div>
        )}
      </div>
...
export default RejectedResignations;
