    ...
    <div className="emp-dash">
      <main className="p-4 sm:p-6 lg:p-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6">
          ...
          <div className="emp-dash__stat">
            <div className="emp-dash__stat-top">
              <span className="emp-dash__stat-label">Active Check-ins</span>
              <div className="emp-dash__stat-icon emp-dash__stat-icon--present">
                <FiUserCheck />
              </div>
            </div>
            <div className="emp-dash__stat-value">{checkedInCount}</div>
            <div className="emp-dash__stat-meta">currently working</div>
          </div>

          <div className="emp-dash__stat">
            <div className="emp-dash__stat-top">
              <span className="emp-dash__stat-label">Checked Out</span>
              <div className="emp-dash__stat-icon emp-dash__stat-icon--absent">
                <FiUserCheck />
              </div>
            </div>
            <div className="emp-dash__stat-value">{checkedOutCount}</div>
            <div className="emp-dash__stat-meta">completed sessions</div>
          </div>
          ...
          <div className="emp-dash__stat">
            ...
            <div className="emp-dash__stat-value">{onsiteCount}</div>
            <div className="emp-dash__stat-meta">in-office entries</div>
          </div>
        </div>
        ...
        <div className="emp-dash__card mb-6">
          <div className="emp-dash__card-header flex-col sm:flex-row gap-3">
            <div>
              ...
              <p className="emp-dash__card-desc">Search employees by ID/name, department, designation, or date range.</p>
            </div>
          </div>
          ...
          <div className="emp-dash__card-body bg-gray-50/50">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 items-end">
              ...
              <div className="flex flex-col gap-1.5 relative" ref={departmentFilterRef}>
                ...
                {showDepartmentFilter && (
                  <div className="absolute left-0 right-0 top-full z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto">
                    <div onClick={() => { setFilterDepartment(''); setShowDepartmentFilter(false); }} className="px-3 py-2 text-xs font-medium text-gray-500 border-b border-gray-100 cursor-pointer hover:bg-blue-50">
                      All Departments
                    </div>
                    {uniqueDepartments.map(dept => (
                      <div
                        ...
                        className={`px-3 py-2 text-xs cursor-pointer transition-all hover:bg-blue-50 ${
                          filterDepartment === dept ? 'bg-blue-50 text-blue-700 font-semibold' : 'text-gray-700'
                        }`}
                      >
                        {dept}
              ...
              <div className="flex flex-col gap-1.5 relative" ref={designationFilterRef}>
                ...
                {showDesignationFilter && (
                  <div className="absolute left-0 right-0 top-full z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto">
                    <div onClick={() => { setFilterDesignation(''); setShowDesignationFilter(false); }} className="px-3 py-2 text-xs font-medium text-gray-500 border-b border-gray-100 cursor-pointer hover:bg-blue-50">
                      All Designations
                    </div>
                    {uniqueDesignations.map(des => (
                      <div
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  onChange={(e) => setSelectedMonth(e.target.value)}
                  onClick={(e) => e.target.showPicker && e.target.showPicker()}
                  className="h-9 px-3 py-2 text-xs border border-gray-300 bg-white text-gray-900 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                />
              </div>
              <button
                onClick={fetchAttendanceData}
                className="mt-auto h-9 px-4 text-xs font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-all shadow-sm"
              >
                Apply
              </button>
            </div>
            ...
            <>
                ...
                <table className="emp-dash__table">
                  ...
                  <tbody>
                    {currentRows.map((rec) => {
                      ...
                      const empObj = employees.find(e => e.employeeId === rec.employeeId || e._id === rec.employeeId);

                      return (
                        <tr key={rec._id} className="transition-colors hover:bg-slate-50/50">
                          <td className="px-3 py-3 font-semibold text-slate-800 whitespace-nowrap text-[11px]">
                            {rec.employeeId || '-'}
                          </td>
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <div
                                style={{ display: empObj?.profilePicture ? 'none' : 'flex' }}
                                className="items-center justify-center w-7 h-7 text-[10px] font-bold bg-gradient-to-br from-indigo-500 to-blue-600 text-white rounded-full shadow-inner flex-shrink-0"
                              >
                                {rec.name ? rec.name.charAt(0).toUpperCase() : '?'}
                              </div>
                              <span className="font-semibold text-slate-800 text-xs whitespace-nowrap">{rec.name}</span>
                            </div>
                          </td>
                          <td className="px-3 py-3 text-slate-600 text-[11px] font-medium whitespace-nowrap">{rec.department}</td>
                          <td className="px-3 py-3 text-slate-600 text-[11px] font-medium whitespace-nowrap">{rec.designation}</td>
                          ...
                          <td className="px-3 py-3 text-center whitespace-nowrap">
                            {rec.checkInTime ? (
                              <span className="text-[11px] font-semibold text-slate-700 px-1.5 py-0.5 bg-slate-100 rounded border border-slate-200">
                                {formatTimeWithAMPM(rec.checkInTime)}
                              </span>
                            ) : <span className="text-slate-400">-</span>}
                          </td>
                          <td className="px-3 py-3 text-center whitespace-nowrap">
                            {rec.checkOutTime ? (
                              <span className="text-[11px] font-semibold text-indigo-700 px-1.5 py-0.5 bg-indigo-50 rounded border border-indigo-100">
                                {formatTimeWithAMPM(rec.checkOutTime)}
                              </span>
                            ) : <span className="text-slate-400">-</span>}
                          </td>
                          <td className="px-3 py-3 text-center whitespace-nowrap">
                          ...
                          <td className="px-3 py-3 text-center whitespace-nowrap">
                            {isCheckedIn ? (
                              <div className="flex items-center justify-center gap-1.5">
                                ...
                                <span className="text-[10px] font-bold text-emerald-600 px-2 py-0.5 bg-emerald-50 rounded border border-emerald-100">
                                  Active
                                </span>
                              </div>
                            ) : (
                              <span className="emp-dash__table-status emp-dash__table-status--other">
                                Logged Out
                              </span>
                            )}
                          </td>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-t border-gray-200/50 bg-gray-50/30">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div className="text-xs text-gray-500 font-medium">
                    Showing <strong className="text-gray-800">{indexOfFirstRow + 1} – {Math.min(indexOfLastRow, filteredRecords.length)}</strong> of{' '}
