// DoctorReferralOP.js — All Doctors + Their Referred OP Bookings
...
import {
  ...
  FiCalendar, FiChevronDown, FiAlertCircle, FiAward
} from "react-icons/fi";
...
const getDoctorPayable = (doctor, booking) => {
...
const getDoctorPayableBreakdown = (doctor, booking) => {
  if (!doctor || !booking) return { total: 0, items: [] };
  const services = getBookingServices(booking);
...
const getAppliedCategories = (doctor, booking) => {
...
export default function DoctorReferralOP() {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(() => {
    const saved = localStorage.getItem("doctorReferralOP_itemsPerPage");
  ...
  const [selectedBookingForDoctorModal, setSelectedBookingForDoctorModal] = useState(null);

  const [showDoctorPaymentModal, setShowDoctorPaymentModal] = useState(false);
  const [selectedBookingForDoctorPayment, setSelectedBookingForDoctorPayment] = useState(null);
  const [selectedDoctorForPayment, setSelectedDoctorForPayment] = useState(null);
  const [newDoctorPaymentStatus, setNewDoctorPaymentStatus] = useState("Pending");
  const [savingDoctorPayment, setSavingDoctorPayment] = useState(false);
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const doctorReferrals = allReferrals.filter((r) => r.referralType === "doctor");
      ...
      console.error("Error fetching doctor referral OP data:", err);
  ...
  const buildFlatRows = () => {
    ...
    referrals.forEach((ref) => {
      ...
      if (refBookings.length === 0) {
        rows.push({ doctor: ref, booking: null });
      } else {
        refBookings.forEach((b) => {
          rows.push({ doctor: ref, booking: b });
        });
      }
    });
    return rows;
  ...
  const uniqueDoctors = useMemo(() => {
    const map = new Map();
    referrals.forEach((r) => {
      if (r.doctorName) map.set(r._id, r.doctorName);
    });
    return Array.from(map.entries()).map(([id, name]) => ({ id, name }));
  }, [referrals]);

  const filteredRows = useMemo(() => {
    return flatRows.filter(({ doctor, booking }) => {
      if (doctorFilter !== "All" && doctor._id !== doctorFilter) return false;
      ...
      if (!booking) {
        ...
        if (searchQuery.trim()) {
          ...
          const m =
            ...
            (doctor.doctorOrganization || "").toLowerCase().includes(q) ||
            (doctor.doctorPhone || "").toLowerCase().includes(q) ||
            (doctor.doctorSpecialization || "").toLowerCase().includes(q);
          if (!m) return false;
        }
        return true;
      }
  ...
  }, [flatRows, doctorFilter, statusFilter, searchQuery, fromDate, toDate, selectedMonth]);

  useEffect(() => { setCurrentPage(1); }, [searchQuery, statusFilter, doctorFilter, fromDate, toDate, selectedMonth]);

  // ✅ FIXED STATS — totalPayable is now SUM of doctor payables, not patient total
  const stats = useMemo(() => {
    const totalDoctors = referrals.length;
    ...
    // ✅ Sum doctor payable per booking (not patient total)
    flatRows.forEach(({ doctor, booking }) => {
      if (doctor && booking) {
        payableTotal += getDoctorPayable(doctor, booking);
      }
    });
  ...
  const handleItemsPerPageChange = (e) => {
    ...
    setItemsPerPage(v);
    localStorage.setItem("doctorReferralOP_itemsPerPage", String(v));
    setCurrentPage(1);
  };
  ...
  const downloadCSV = () => {
    ...
    const headers = [
      "#", "Doctor Name", "Organization", "Doctor Phone", "Specialization",
      "Clinic %", "Pharmacy %", "Lab %",
      "Patient", "Phone", "Appt. Date", "Slot Timing",
      "Services", "Applied On", "Total", "Payment Status", "Doctor Payable", "Doctor Payment Status",
      "Payment Mode", "Created At"
    ];
    const csvRows = [headers.join(","), ...filteredRows.map((row, idx) => {
      const { doctor, booking } = row;
      if (!booking) {
        return [
          idx + 1,
          `"${(doctor.doctorName || "").replace(/"/g, '""')}"`,
          `"${(doctor.doctorOrganization || "").replace(/"/g, '""')}"`,
          `"${doctor.doctorPhone || ""}"`,
          `"${(doctor.doctorSpecialization || "").replace(/"/g, '""')}"`,
          `"${doctor.clinicCommission || 0}"`,
          `"${doctor.pharmacyCommission || 0}"`,
          `"${doctor.labCommission || 0}"`,
          `"-"`, `"-"`, `"-"`, `"-"`,
          `"-"`, `"-"`, 0, `"-"`, 0, `"${doctor.doctorPaymentStatus || "Pending"}"`, `"-"`, `"-"`
        ].join(",");
      }
      const info = getBookingPaidInfo(booking);
      const services = getBookingServices(booking);
      const doctorPayable = getDoctorPayable(doctor, booking);
      const appliedCats = getAppliedCategories(doctor, booking);
      return [
        idx + 1,
        `"${(doctor.doctorName || "").replace(/"/g, '""')}"`,
        `"${(doctor.doctorOrganization || "").replace(/"/g, '""')}"`,
    ...
    a.href = url;
    a.download = `Doctor_Referral_OP_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    URL.revokeObjectURL(url);
    showToast(`Exported ${filteredRows.length} records!`);
  };

  const openDoctorPaymentModal = (doctor, booking) => {
    setSelectedDoctorForPayment(doctor);
    setSelectedBookingForDoctorPayment(booking);
    setNewDoctorPaymentStatus(booking?.doctorPaymentStatus || "Pending");
    setShowDoctorPaymentModal(true);
  };

  const handleSaveDoctorPaymentStatus = async () => {
    if (!selectedBookingForDoctorPayment) {
      showToast("No booking selected", "error");
      return;
    }
    setSavingDoctorPayment(true);
    try {
      const res = await axios.put(
        `${BOOKINGS_API}/updatedoctorpayment/${selectedBookingForDoctorPayment._id}`,
        { doctorPaymentStatus: newDoctorPaymentStatus }
      );
      if (res.data?.success || res.status === 200) {
        setBookings((prev) =>
          prev.map((b) =>
            b._id === selectedBookingForDoctorPayment._id
              ? { ...b, doctorPaymentStatus: newDoctorPaymentStatus }
              : b
          )
        );
        showToast(`Doctor payment status updated to ${newDoctorPaymentStatus}!`, "success");
        setShowDoctorPaymentModal(false);
        setSelectedDoctorForPayment(null);
        setSelectedBookingForDoctorPayment(null);
      } else {
        showToast(res.data?.message || "Failed to update", "error");
      }
    } catch (err) {
      console.error("Error updating doctor payment status:", err);
      showToast(err.response?.data?.message || "Failed to update doctor payment status", "error");
    } finally {
      setSavingDoctorPayment(false);
    }
  };
      ...
      <main className="p-2 sm:p-4 lg:p-6">
        ...
        <div className="hidden lg:flex items-center justify-between gap-3 flex-wrap mb-6">
          <h1 className="emp-dash__greeting text-lg sm:text-xl font-bold whitespace-nowrap">
            Doctor Referral <span>OP</span>
          </h1>
          <div className="flex items-center gap-2 flex-wrap">
            <div className="emp-dash__date-pill">
              <FaUserMd />
              <span>{referrals.length} Doctors • {bookings.length} Bookings</span>
            </div>
            ...
            <select value={doctorFilter} onChange={(e) => setDoctorFilter(e.target.value)} className="h-8 px-2 py-1 text-xs border border-gray-300 bg-white text-gray-900 rounded-lg max-w-[150px] truncate">
        ...
        <div className="lg:hidden flex items-center justify-between gap-2 flex-wrap mb-3">
          <div className="flex items-baseline gap-2">
            <h1 className="text-base font-bold whitespace-nowrap">Doctor Referral <span className="text-purple-600">OP</span></h1>
            <div className="emp-dash__date-pill text-[10px] px-2 py-1">
              <FaUserMd className="w-3 h-3 text-purple-600" />
              <span>{referrals.length}D • {bookings.length}B</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-3 md:gap-4 mb-6">
          <div className={`emp-dash__stat cursor-pointer hover:scale-105 ${activeCardFilter === "all" ? "ring-2 ring-blue-500/20 border-blue-400" : ""}`} onClick={() => handleCardClick("all")}>
            <div className="emp-dash__stat-top"><span className="emp-dash__stat-label">Total Doctors</span><div className="emp-dash__stat-icon emp-dash__stat-icon--rate"><FiUsers /></div></div>
            <div className="emp-dash__stat-value">{stats.totalDoctors}</div>
            <div className="emp-dash__stat-meta">referring doctors</div>
          </div>
          ...
          {/* ✅ FIXED — Now shows Doctor Payable total, not patient revenue */}
          <div className="emp-dash__stat">
            <div className="emp-dash__stat-top">
              <span className="emp-dash__stat-label">Total Doctor Payable</span>
              <div className="emp-dash__stat-icon emp-dash__stat-icon--present"><FaRupeeSign /></div>
            </div>
            <div className="emp-dash__stat-value text-purple-700">₹{stats.totalPayable.toLocaleString()}</div>
            <div className="emp-dash__stat-meta">payable to doctors</div>
          </div>
        </div>
        ...
        <div className="emp-dash__card">
          {loading ? (
            <div className="py-12 text-center"><FiRefreshCw className="w-8 h-8 text-purple-600 animate-spin mx-auto mb-3" /><p className="text-sm text-gray-500">Loading doctor referral OP records...</p></div>
          ) : filteredRows.length === 0 ? (
            <div className="py-12 text-center">
              <FaUserMd className="w-12 h-12 text-gray-300 mx-auto mb-3" />
              <h3 className="text-base font-bold text-gray-700">No Doctor Referral OP Records</h3>
                ...
                <table className="emp-dash__table">
                  <thead>
                    <tr>
                      ...
                      <th style={{ textAlign: "center" }}>Doctor Payable</th>
                      <th style={{ textAlign: "center" }}>Doctor Payment Status</th>
                  ...
                  <tbody>
                    {currentRows.map((row, idx) => {
                      ...
                      if (!booking) {
                        return (
                          <tr key={`${doctor._id}-no-booking`} className="hover:bg-purple-50/30">
                            <td className="px-2 py-3 text-center text-slate-500 text-[11px]">{indexOfFirstItem + idx + 1}</td>
                            <td className="px-3 py-3">
                              <div className="flex items-center gap-2.5">
                                <div className="w-8 h-8 rounded-full bg-purple-500 text-white font-bold flex items-center justify-center text-[10px]">
                                  {doctor.doctorName ? doctor.doctorName.charAt(0).toUpperCase() : "D"}
                                </div>
                                <div>
                                  <div className="font-semibold text-xs text-purple-800 truncate max-w-[120px]">{doctor.doctorName || "N/A"}</div>
                                  <div className="text-[9px] text-gray-400 truncate max-w-[120px]">{doctor.doctorOrganization || "-"}</div>
                                </div>
                              </div>
                            </td>
                      ...
                      const doctorPayable = getDoctorPayable(doctor, booking);
                      const doctorPaymentStatus = booking.doctorPaymentStatus || "Pending";
                      const doctorPayColors = getPaymentStatusColors(doctorPaymentStatus);
                      const appliedCats = getAppliedCategories(doctor, booking);

                      return (
                        <tr key={booking._id} className="hover:bg-purple-50/30">
                          <td className="px-2 py-3 text-center text-slate-500 text-[11px]">{indexOfFirstItem + idx + 1}</td>
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-2.5">
                              <div className="w-8 h-8 rounded-full bg-purple-500 text-white font-bold flex items-center justify-center text-[10px]">
                          ...
                          <td className="px-3 py-3 text-center whitespace-nowrap">
                            <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase border ${doctorPayColors.bg} ${doctorPayColors.text} ${doctorPayColors.border}`}>
                              <doctorPayColors.icon className={`w-2.5 h-2.5 ${doctorPayColors.iconColor}`} /> {doctorPaymentStatus}
                            </span>
                          </td>
                          <td className="px-3 py-3 text-center whitespace-nowrap">
                            <div className="flex flex-col gap-0.5 items-center">
                              <span className="text-[9px] font-bold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100" title="Clinic">
                                Clinic: {doctor.clinicCommission || 0}%
                              </span>
                            ...
                            <div className="flex items-center justify-end gap-1.5">
                              <button
                                onClick={() => {
                                  ...
                                  setSelectedBookingForDoctorModal(booking);
                              ...
                              <button
                                onClick={() => openDoctorPaymentModal(doctor, booking)}
                                className="p-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg"
                                title="Edit Doctor Payment"
                              >
          ...
          <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl border max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between pb-4 border-b">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-purple-600 text-white flex items-center justify-center"><FaUserMd /></div>
                  <div>
                    <h3 className="font-bold text-gray-900">Doctor Referral Details</h3>
                ...
                <button
                  onClick={() => {
                    ...
                    setSelectedBookingForDoctorModal(null);
                  }}
              ...
              <div className="my-4 bg-gray-50 p-5 rounded-xl border space-y-4">
                <div className="flex items-center gap-3 pb-3 border-b">
                  <div className="w-14 h-14 rounded-full bg-purple-500 text-white font-bold text-xl flex items-center justify-center">
                    {selectedDoctor.doctorName?.charAt(0).toUpperCase() || "D"}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-gray-900 text-base">{selectedDoctor.doctorName}</div>
                    <div className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
                      <FaBuilding className="text-[10px]" />
                      {selectedDoctor.doctorOrganization || "N/A"}
                    </div>
                  </div>
                  <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase border ${selectedDoctor.status === "active" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-red-50 text-red-700 border-red-200"}`}>
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    ...
                    <div className="font-semibold">{selectedDoctor.doctorPhone || "N/A"}</div>
                  </div>
                  <div>
                    <div className="text-[10px] font-bold uppercase text-gray-400">Specialization</div>
                    <div className="font-semibold">{selectedDoctor.doctorSpecialization || "General"}</div>
                  </div>
                  <div>
                    <div className="text-[10px] font-bold uppercase text-gray-400">Referral Date</div>
                    <div className="font-semibold">{formatDateToDDMMYYYY(selectedDoctor.referralDate || selectedDoctor.createdAt)}</div>
                  </div>
                  <div>
                    <div className="text-[10px] font-bold uppercase text-gray-400">Created</div>
                    <div className="font-semibold">{formatDateTimeToDDMMYYYY(selectedDoctor.createdAt)}</div>
                  </div>
                </div>

                <div className="pt-3 border-t">
                  <div className="text-[10px] font-bold uppercase text-gray-400 mb-2 flex items-center gap-1">
                    <FiAward className="text-purple-500" /> Referral % Breakdown
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    <div className="bg-blue-50 p-2.5 rounded-lg text-center border border-blue-100">
                      ...
                      <div className="text-base font-extrabold text-blue-900">{selectedDoctor.clinicCommission || 0}%</div>
                    </div>
                    <div className="bg-green-50 p-2.5 rounded-lg text-center border border-green-100">
                      ...
                      <div className="text-base font-extrabold text-green-900">{selectedDoctor.pharmacyCommission || 0}%</div>
                    </div>
                    <div className="bg-purple-50 p-2.5 rounded-lg text-center border border-purple-100">
                      ...
                      <div className="text-base font-extrabold text-purple-900">{selectedDoctor.labCommission || 0}%</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-3 border-t">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="bg-blue-50 p-2 rounded-lg text-center border border-blue-100">
                      ...
                      <div className="text-base font-extrabold text-blue-900">{getBookingsForReferral(selectedDoctor).length}</div>
                    </div>
                    <div className="bg-emerald-50 p-2 rounded-lg text-center border border-emerald-100">
                      ...
                      <div className="text-base font-extrabold text-emerald-900">
                        ₹{getBookingsForReferral(selectedDoctor).reduce((sum, b) => sum + getBookingFinalPayable(b), 0).toLocaleString()}
                      </div>
                    </div>
                    <div className="bg-purple-50 p-2 rounded-lg text-center border border-purple-100">
                      <div className="text-[9px] text-purple-600 font-bold">Doctor Payable</div>
                      <div className="text-base font-extrabold text-purple-900">
                        ₹{getBookingsForReferral(selectedDoctor).reduce((sum, b) => sum + getDoctorPayable(selectedDoctor, b), 0).toLocaleString()}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-3 border-t">
                  <div className="text-[10px] font-bold uppercase text-gray-400 mb-1">Doctor Payment Status</div>
                  <span className={`inline-block text-[11px] font-bold px-3 py-1 rounded-full uppercase border ${getPaymentStatusColors(selectedBookingForDoctorModal?.doctorPaymentStatus || "Pending").bg} ${getPaymentStatusColors(selectedBookingForDoctorModal?.doctorPaymentStatus || "Pending").text} ${getPaymentStatusColors(selectedBookingForDoctorModal?.doctorPaymentStatus || "Pending").border}`}>
                    {selectedBookingForDoctorModal?.doctorPaymentStatus || "Pending"}
                  </span>
                </div>

                {selectedBookingForDoctorModal && (() => {
                  const breakdown = getDoctorPayableBreakdown(selectedDoctor, selectedBookingForDoctorModal);
                  const appliedCats = getAppliedCategories(selectedDoctor, selectedBookingForDoctorModal);
                  return (
                    <div className="pt-3 border-t">
                      ...
                      <div className="bg-white p-3 rounded-lg border border-gray-200 space-y-1.5 text-xs">
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold">{selectedBookingForDoctorModal.patientTitle} {selectedBookingForDoctorModal.patientName}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">Phone:</span>
                          <span className="font-semibold">{selectedBookingForDoctorModal.patientPhone || "N/A"}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">Appt. Date:</span>
                          <span className="font-semibold">{formatDateToDDMMYYYY(selectedBookingForDoctorModal.appointmentDate || selectedBookingForDoctorModal.date)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">Treating Doctor:</span>
                          <span className="font-semibold">{selectedBookingForDoctorModal.doctorName || "N/A"}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">Total Amount:</span>
                          <span className="font-semibold">₹{Math.round(getBookingFinalPayable(selectedBookingForDoctorModal))}</span>
                        </div>
                        <div className="flex justify-between">
                            ...
                            <tbody>
                              {breakdown.items.map((it, i) => (
                                <tr key={i} className="border-t border-gray-100">
                                  ...
                                  <td className="px-2 py-1.5 text-right font-bold text-purple-700">₹{it.payable}</td>
                                </tr>
                              ))}
                            </tbody>
                            <tfoot className="bg-purple-50 border-t-2 border-purple-200">
                              <tr>
                                <td colSpan={4} className="px-2 py-2 text-right font-bold text-purple-800 text-[11px]">
                                  Total Doctor Payable
                                </td>
                                <td className="px-2 py-2 text-right font-extrabold text-purple-900 text-[12px]">
                                  ₹{breakdown.total}
                                </td>
                              </tr>
                            </tfoot>
              ...
              <div className="flex justify-end gap-2">
                <button
                  onClick={() => {
                    setShowDoctorModal(false);
                    openDoctorPaymentModal(selectedDoctor, selectedBookingForDoctorModal);
                  }}
        ...
        {/* Doctor Payment Status Update Modal */}
        {showDoctorPaymentModal && selectedDoctorForPayment && (
            ...
            <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border">
              <div className="flex items-center justify-between pb-4 border-b">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-gray-900 text-base">Update Doctor Payment</h3>
                    <p className="text-xs text-gray-500">{selectedDoctorForPayment.doctorName}</p>
                  </div>
                </div>
                <button
                  onClick={() => { setShowDoctorPaymentModal(false); setSelectedDoctorForPayment(null); setSelectedBookingForDoctorPayment(null); }}
              ...
              {selectedBookingForDoctorPayment && (
                <div className="mt-4 p-3 bg-purple-50 rounded-lg border border-purple-200">
                  <div className="text-xs text-purple-700 font-bold uppercase">Doctor Payable For This Booking</div>
                  <div className="text-lg font-extrabold text-purple-900 mt-0.5">
                    ₹{getDoctorPayable(selectedDoctorForPayment, selectedBookingForDoctorPayment)}
                  </div>
                  <div className="text-[10px] text-purple-600 mt-0.5">
                    Patient: {selectedBookingForDoctorPayment.patientName} • Total: ₹{Math.round(getBookingFinalPayable(selectedBookingForDoctorPayment))}
                  </div>
                </div>
              )}
              ...
              <div className="my-5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {["Pending", "Paid"].map((st) => {
                    const isSelected = newDoctorPaymentStatus === st;
                    const colors = getPaymentStatusColors(st);
                    return (
                      <button
                        key={st}
                        type="button"
                        onClick={() => setNewDoctorPaymentStatus(st)}
              ...
              <div className="flex justify-end gap-3 pt-3 border-t">
                ...
                <button
                  onClick={handleSaveDoctorPaymentStatus}
                  disabled={savingDoctorPayment}
                ...
                >
                  {savingDoctorPayment ? <FiRefreshCw className="w-3.5 h-3.5 animate-spin" /> : <FiCheckCircle className="w-3.5 h-3.5" />}
                  {savingDoctorPayment ? "Saving..." : "Save Status"}
                </button>
              </div>
            </div>
          </div>
